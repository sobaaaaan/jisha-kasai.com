  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("cases").insert({
        prefecture_id: null,        // TODO: prefecture名→IDの変換（マスターテーブル参照）
        ...
        description_masked: false,  // 管理者が後でマスク処理
        source_url:     form.source_url || null,
        source_name:    null,
        is_published:   false,      // 管理者承認待ち
        // submitter_comment は cases テーブルに列がないため別途保存（将来）
        // 現時点では description の末尾に付記する運用
      });
