...
interface CaseRow {
  ...
  Facility_category: CategoryKey | string | null;
  status: string | null;
...
const Facility_CATEGORY_LABELS: Record<CategoryKey, string> = {
  violent: "暴力犯罪",
  property: "財産犯罪",
  sexual: "性犯罪",
  drug: "薬物",
...
async function fetchCasePoints(categoryFilter: CategoryFilter): Promise<CasePoint[]> {
  let query = supabase
    .from("cases")
    .select(
      "id, latitude, longitude, occurred_year, occurred_month, Facility_category, status, description, source_url, submitter_comment"
    )
  ...
  if (categoryFilter !== "all") {
    query = query.eq("Facility_category", categoryFilter);
  }
  ...
  return (data as CaseRow[])
    ...
    .map((row) => ({
      ...
      Facility_category: normalizeCategory(row.Facility_category),
      status: row.status,
...
function buildPopupHTML(item: CasePoint): string {
  const categoryLabel = Facility_CATEGORY_LABELS[item.Facility_category] ?? "その他";
  const occurred =
...
export default function CaseMap() {
  ...
  useEffect(() => {
    ...
    cases.forEach((item) => {
      const color = CATEGORY_COLORS[item.Facility_category] ?? CATEGORY_COLORS.other;
        ...
        <div style={{ padding: "18px 18px 24px" }}>
          ...
          <section style={cardStyle}>
            ...
            >
              ...
              {Object.entries(Facility_CATEGORY_LABELS).map(([k, label]) => (
                ...
                {latestCases.map((item) => (
                  ...
                  >
                    ...
                    <div style={{ color: "#777", fontSize: 12 }}>
                      {Facility_CATEGORY_LABELS[item.Facility_category]} / {statusLabel(item.status)}
                    </div>
                  </li>
                ))}
              </ul>
